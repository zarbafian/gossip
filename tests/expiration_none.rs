...
fn all_updates_received() {
    ...
    let _ = service.start(no_peer_handler, Box::new(MapUpdatingHandler::new(init_peer.to_owned(), Arc::clone(&peer_messages))));
    instances.push(service);

    let mut port = 9001;
    for _i in 1..peer_count {
        // peer socket address
        ...
        let _ = ipv4_service.start(init_handler, Box::new(MapUpdatingHandler::new(address.clone(), Arc::clone(&peer_messages))));
    ...
    for i in 0..message_count {
        ...
        let _  = instances[selected_peer].submit(message);
        {
